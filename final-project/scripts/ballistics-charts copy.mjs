export function convertTrajectoryData(calculatorData) {
    ...
    return calculatorData.map(data => ({
        distance: Math.round(data.x / 3), // Convert feet to yards
        drop: -data.y // Convert to negative drop (inches)
    }));
}
...
export function formatTrajectoryData(originalTrajectoryData) {
    ...
    originalTrajectoryData.forEach(data => {
        trajectoryData.push({ distance: data.distance, drop: -Math.abs(data.drop) });
    });
